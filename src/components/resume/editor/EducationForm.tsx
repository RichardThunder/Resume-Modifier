...
// --- DraggableItem Component (Example using native HTML DnD) ---
...
const DraggableItem: React.FC<DraggableItemProps> = ({ id, index, children, onDragEnd }) => {
    const [isDragging, setIsDragging] = useState(false);
    const dragCounter = useRef(0); // To handle nested drag enter/leave
    ...
    const handleDragEnd = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        // Find drop target index (logic depends on setup, often done in parent drop handler)
    };
    ...
    const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (draggedIndexStr) {
            const startIndex = parseInt(draggedIndexStr, 10);
            const endIndex = index; // The index of the item being dropped on
            if (!isNaN(startIndex)) {
                onDragEnd(startIndex, endIndex);
            }
        }
        // Remove drag-over class from target
        e.currentTarget.classList.remove('border-blue-400', 'border-2'); // Example class
    };

    const handleDragEnter = (e: React.DragEvent<HTMLDivElement>) => {
        e.preventDefault();
        dragCounter.current++;
        // Add visual cue only if dragging over a different item
        const draggedIndexStr = e.dataTransfer.getData('text/plain'); // Might not be available here reliably
        // Simpler: Add class on enter if not the source
        // This check is tricky without access to the source index directly here
        // A common pattern is to handle visual cues based on comparing the target index with a stored source index (e.g., in parent state or context)
        e.currentTarget.classList.add('border-blue-400', 'border-2'); // Example class
    };
    ...
    const handleDragLeave = (e: React.DragEvent<HTMLDivElement>) => {
        ...
        if (dragCounter.current === 0) {
            // Remove visual cue only when leaving the element completely
            e.currentTarget.classList.remove('border-blue-400', 'border-2'); // Example class
        }
    };
    ...
    return (
        <div
            ...
            onDrop={handleDrop}
            onDragEnter={handleDragEnter} // Add visual cue on enter
            onDragLeave={handleDragLeave} // Remove visual cue on leave
            // Add dynamic classes based on isDragging state if needed
            className={`${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
        >
            {children(isDragging)} {/* Pass dragging state to children */}
        </div>
    );
};