...
import { Volunteering } from '@/types/resume'; // Import the Volunteering type
...
const VolunteeringForm: React.FC = () => {
    const { volunteering, addVolunteering, deleteVolunteering, reorderVolunteering, updateVolunteering, analysis, model } = useResumeStore(state => ({
        volunteering: state.model.volunteering,
        addVolunteering: state.addVolunteering,
        deleteVolunteering: state.deleteVolunteering,
        reorderVolunteering: state.reorderVolunteering,
        updateVolunteering: state.updateVolunteering,
        analysis: state.analysis.volunteering, // Get volunteering analysis
        model: state.model,
    }));
    ...
    const handleAddVolunteering = () => {
        addVolunteering();
        const newIndex = volunteering.length;
        setVisibleIndexes(prev => ({ ...prev, [newIndex]: true }));
        setShowDragHint(true);
        setTimeout(() => setShowDragHint(false), 3000);
    };

    const handleDeleteVolunteering = (index: number, event: React.MouseEvent) => {
        event.stopPropagation();
        if (window.confirm(`Are you sure you want to delete Volunteering #${index + 1}?`)) {
    ...
    const handleInputChange = (index: number, e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        updateVolunteering(index, { [name]: fieldValue });
    };

    const onDragEnd = (startIndex: number, endIndex: number) => {
        if (startIndex === endIndex) return;
        reorderVolunteering(startIndex, endIndex);
        setVisibleIndexes(prev => {
    ...
    const handleUpdateDescription = (index: number, newDescription: string) => {
        updateVolunteering(index, { description: newDescription });
    };
    ...
    return (
        <div className="mb-6 mx-auto w-full">
            <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={handleAddVolunteering} className="btn-custom btn-sm">Add</button>
            </div>
            ...
            <div className="space-y-2">
                {volunteering.map((vol, index) => (
                    <DraggableItem key={vol.name + index} id={index.toString()} index={index} onDragEnd={onDragEnd}>
                        {(isDragging) => (
                            <div className={`card mb-1 border ${isDragging ? 'opacity-50 shadow-lg' : 'shadow-sm'}`}>
                                <div className="card-header flex justify-between items-center p-2 cursor-pointer bg-gray-50 hover:bg-gray-100" onClick={() => toggleShow(index)} aria-expanded={visibleIndexes[index]}>
                                    <span className="font-medium truncate pr-2">{vol.role || `Volunteering #${index + 1}`} at {vol.name || 'Organization'}</span>
                                    <div className="flex items-center space-x-2 flex-shrink-0">
                                        ...
                                        <button onClick={(e) => handleDeleteVolunteering(index, e)} className="p-1 text-gray-400 hover:text-red-600" title="Delete Volunteering">
                                ...
                                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${visibleIndexes[index] ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                                    <div className="card-body p-3 space-y-3">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div><label htmlFor={`vol-name-${index}`} className="form-label">Organization/Event Name</label><input type="text" id={`vol-name-${index}`} name="name" value={vol.name || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Organization Name" className="form-control form-control-sm" /></div>
                                            <div><label htmlFor={`role-${index}`} className="form-label">Your Role</label><input type="text" id={`role-${index}`} name="role" value={vol.role || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Your Role" className="form-control form-control-sm" /></div>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div><label htmlFor={`vol-city-${index}`} className="form-label">City</label><input type="text" id={`vol-city-${index}`} name="city" value={vol.city || ''} onChange={(e) => handleInputChange(index, e)} placeholder="City" className="form-control form-control-sm" /></div>
                                            <div><label htmlFor={`vol-country-${index}`} className="form-label">Country</label><input type="text" id={`vol-country-${index}`} name="country" value={vol.country || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Country" className="form-control form-control-sm" /></div>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div><label htmlFor={`vol-fromDate-${index}`} className="form-label">From Date</label><input type="month" id={`vol-fromDate-${index}`} name="fromDate" value={vol.fromDate || ''} onChange={(e) => handleInputChange(index, e)} className="form-control form-control-sm" /></div>
                                            <div><label htmlFor={`vol-toDate-${index}`} className="form-label">To Date</label><input type="month" id={`vol-toDate-${index}`} name="toDate" value={vol.toDate || ''} onChange={(e) => handleInputChange(index, e)} className="form-control form-control-sm" disabled={vol.isPresent} /></div>
                                        </div>
                                        <div className="flex items-center mt-1">
                                            <input type="checkbox" id={`isPresent-vol-${index}`} name="isPresent" checked={vol.isPresent || false} onChange={(e) => handleInputChange(index, e)} className="form-check-input" />
                                            <label htmlFor={`isPresent-vol-${index}`} className="form-check-label ml-2">Currently Volunteering Here</label>
                                        </div>
                                        <div>
                                            <label htmlFor={`description-vol-${index}`} className="form-label">Description</label>
                                            <textarea id={`description-vol-${index}`} name="description" value={vol.description || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Describe the volunteering work..." rows={4} className="form-control form-control-sm min-h-[100px]"></textarea>
                                ...
                                {feedbackModalIndex === index && <FeedbackModal isOpen={true} onClose={closeFeedbackModal} sectionData={vol} sectionType="volunteering" currentResumeModel={model} onUpdateContent={(newDesc) => handleUpdateDescription(index, newDesc)} />}
                            </div>
                        )}
                    </DraggableItem>
                ))}
                {volunteering.length === 0 && <p className="text-center text-gray-500 py-4">No volunteering entries added yet.</p>}
            </div>
        </div>
    );
};
