...
import { Publication } from '@/types/resume'; // Import the Publication type
import { scoreToColors } from '@/lib/methods';
// import FeedbackModal from '@/components/common/FeedbackModal'; // Feedback might not apply here?
...
const PublicationsForm: React.FC = () => {
    const { publications, addPublication, deletePublication, reorderPublication, updatePublication, analysis, model } = useResumeStore(state => ({
        publications: state.model.publications,
        addPublication: state.addPublication,
        deletePublication: state.deletePublication,
        reorderPublication: state.reorderPublication,
        updatePublication: state.updatePublication,
        analysis: state.analysis.publications, // Get publication analysis
        model: state.model,
    }));

    const [visibleIndexes, setVisibleIndexes] = useState<Record<number, boolean>>({});
    const [showDragHint, setShowDragHint] = useState(false);
    // Feedback modal might not be needed for publications, depends on requirements
    ...
    const handleAddPublication = () => {
        addPublication();
        const newIndex = publications.length;
        setVisibleIndexes(prev => ({ ...prev, [newIndex]: true }));
        setShowDragHint(true);
        setTimeout(() => setShowDragHint(false), 3000);
    };

    const handleDeletePublication = (index: number, event: React.MouseEvent) => {
        event.stopPropagation();
        if (window.confirm(`Are you sure you want to delete Publication #${index + 1}?`)) {
    ...
    const handleInputChange = (index: number, e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        ...
        updatePublication(index, { [name]: value });
    };

    const onDragEnd = (startIndex: number, endIndex: number) => {
        if (startIndex === endIndex) return;
        reorderPublication(startIndex, endIndex);
        setVisibleIndexes(prev => {
    ...
    //    updatePublication(index, { description: newDescription });
    // };
    ...
    return (
        <div className="mb-6 mx-auto w-full">
            <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={handleAddPublication} className="btn-custom btn-sm">Add</button>
            </div>
            ...
            <div className="space-y-2">
                {publications.map((pub, index) => (
                    <DraggableItem key={pub.name + index} id={index.toString()} index={index} onDragEnd={onDragEnd}>
                        {(isDragging) => (
                            <div className={`card mb-1 border ${isDragging ? 'opacity-50 shadow-lg' : 'shadow-sm'}`}>
                                <div className="card-header flex justify-between items-center p-2 cursor-pointer bg-gray-50 hover:bg-gray-100" onClick={() => toggleShow(index)} aria-expanded={visibleIndexes[index]}>
                                    <span className="font-medium truncate pr-2">{pub.name || `Publication #${index + 1}`}</span>
                                    <div className="flex items-center space-x-2 flex-shrink-0">
                                        ...
                                        <button onClick={(e) => handleDeletePublication(index, e)} className="p-1 text-gray-400 hover:text-red-600" title="Delete Publication">
                                ...
                                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${visibleIndexes[index] ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                                    <div className="card-body p-3 space-y-3">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div><label htmlFor={`pub-name-${index}`} className="form-label">Publication Title</label><input type="text" id={`pub-name-${index}`} name="name" value={pub.name || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Title of Publication" className="form-control form-control-sm" /></div>
                                            <div><label htmlFor={`publisher-${index}`} className="form-label">Publisher</label><input type="text" id={`publisher-${index}`} name="publisher" value={pub.publisher || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Publisher Name" className="form-control form-control-sm" /></div>
                                        </div>
                                        <div><label htmlFor={`pub-url-${index}`} className="form-label">URL (Optional)</label><input type="url" id={`pub-url-${index}`} name="url" value={pub.url || ''} onChange={(e) => handleInputChange(index, e)} placeholder="https://example.com/publication" className="form-control form-control-sm" /></div>
                                        <div><label htmlFor={`pub-date-${index}`} className="form-label">Date of Publication</label><input type="month" id={`pub-date-${index}`} name="date" value={pub.date || ''} onChange={(e) => handleInputChange(index, e)} className="form-control form-control-sm" /></div>
                                        {/* Optional Description Field */}
                                        {/* <div>
                                            <label htmlFor={`description-pub-${index}`} className="form-label">Description (Optional)</label>
                                            <textarea id={`description-pub-${index}`} name="description" value={pub.description || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Brief description..." rows={2} className="form-control form-control-sm min-h-[60px]"></textarea> */}
                                        {/* Optional AI Writer */}
                                ...
                                {/* Feedback Modal (if description/AI writer is added) */}
                                {/* {feedbackModalIndex === index && <FeedbackModal isOpen={true} onClose={closeFeedbackModal} sectionData={pub} sectionType="publications" currentResumeModel={model} onUpdateContent={(newDesc) => handleUpdateDescription(index, newDesc)} />} */}
                            </div>
                        )}
                    </DraggableItem>
                ))}
                {publications.length === 0 && <p className="text-center text-gray-500 py-4">No publications entries added yet.</p>}
            </div>
        </div>
    );
};
