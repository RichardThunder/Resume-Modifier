...
import { Project } from '@/types/resume'; // Import the Project type
...
const ProjectForm: React.FC = () => {
    const { project, addProject, deleteProject, reorderProject, updateProject, analysis, model } = useResumeStore(state => ({
        project: state.model.project,
        addProject: state.addProject,
        deleteProject: state.deleteProject,
        reorderProject: state.reorderProject,
        updateProject: state.updateProject,
        analysis: state.analysis.project, // Get project analysis
        model: state.model,
    }));
    ...
    }, [project.length]); // Re-run only if item count changes
    ...
    const handleAddProject = () => {
        addProject();
        const newIndex = project.length; // Index before adding
        setVisibleIndexes(prev => ({ ...prev, [newIndex]: true }));
        setShowDragHint(true);
        setTimeout(() => setShowDragHint(false), 3000);
    };

    const handleDeleteProject = (index: number, event: React.MouseEvent) => {
        event.stopPropagation();
        if (window.confirm(`Are you sure you want to delete Project #${index + 1}?`)) {
            deleteProject(index);
    ...
    return (
        <div className="mb-6 mx-auto w-full">
            <div className="flex justify-between items-center mb-3">
                ...
                <button onClick={handleAddProject} className="btn-custom btn-sm">Add</button>
            </div>
            ...
            <div className="space-y-2">
                {project.map((proj, index) => (
                    <DraggableItem key={proj.title + index} id={index.toString()} index={index} onDragEnd={onDragEnd}>
                        {(isDragging) => (
                            <div className={`card mb-1 border ${isDragging ? 'opacity-50 shadow-lg' : 'shadow-sm'}`}>
                                <div className="card-header flex justify-between items-center p-2 cursor-pointer bg-gray-50 hover:bg-gray-100" onClick={() => toggleShow(index)} aria-expanded={visibleIndexes[index]}>
                                    <span className="font-medium truncate pr-2">{proj.title || `Project #${index + 1}`}</span>
                                    <div className="flex items-center space-x-2 flex-shrink-0">
                                        ...
                                        <button onClick={(e) => handleDeleteProject(index, e)} className="p-1 text-gray-400 hover:text-red-600" title="Delete Project">
                                ...
                                <div className={`overflow-hidden transition-all duration-500 ease-in-out ${visibleIndexes[index] ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
                                    <div className="card-body p-3 space-y-3">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div><label htmlFor={`title-${index}`} className="form-label">Project Title</label><input type="text" id={`title-${index}`} name="title" value={proj.title || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Project Title" className="form-control form-control-sm" /></div>
                                            <div><label htmlFor={`projectRole-${index}`} className="form-label">Role in Project</label><input type="text" id={`projectRole-${index}`} name="projectRole" value={proj.projectRole || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Your Role" className="form-control form-control-sm" /></div>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div><label htmlFor={`proj-city-${index}`} className="form-label">City</label><input type="text" id={`proj-city-${index}`} name="city" value={proj.city || ''} onChange={(e) => handleInputChange(index, e)} placeholder="City (Optional)" className="form-control form-control-sm" /></div>
                                            <div><label htmlFor={`proj-country-${index}`} className="form-label">Country</label><input type="text" id={`proj-country-${index}`} name="country" value={proj.country || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Country (Optional)" className="form-control form-control-sm" /></div>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div><label htmlFor={`proj-fromDate-${index}`} className="form-label">From Date</label><input type="month" id={`proj-fromDate-${index}`} name="fromDate" value={proj.fromDate || ''} onChange={(e) => handleInputChange(index, e)} className="form-control form-control-sm" /></div>
                                            <div><label htmlFor={`proj-toDate-${index}`} className="form-label">To Date</label><input type="month" id={`proj-toDate-${index}`} name="toDate" value={proj.toDate || ''} onChange={(e) => handleInputChange(index, e)} className="form-control form-control-sm" disabled={proj.isPresent} /></div>
                                        </div>
                                        <div className="flex items-center mt-1">
                                            <input type="checkbox" id={`isPresent-proj-${index}`} name="isPresent" checked={proj.isPresent || false} onChange={(e) => handleInputChange(index, e)} className="form-check-input" />
                                            <label htmlFor={`isPresent-proj-${index}`} className="form-check-label ml-2">Currently Working on this Project</label>
                                        </div>
                                        <div>
                                            <label htmlFor={`description-proj-${index}`} className="form-label">Description</label>
                                            <textarea id={`description-proj-${index}`} name="description" value={proj.description || ''} onChange={(e) => handleInputChange(index, e)} placeholder="Describe the project, responsibilities, and achievements..." rows={4} className="form-control form-control-sm min-h-[100px]"></textarea>
                                ...
                                {feedbackModalIndex === index && <FeedbackModal isOpen={true} onClose={closeFeedbackModal} sectionData={proj} sectionType="project" currentResumeModel={model} onUpdateContent={(newDesc) => handleUpdateDescription(index, newDesc)} />}
                            </div>
                        )}
                    </DraggableItem>
                ))}
                {project.length === 0 && <p className="text-center text-gray-500 py-4">No project entries added yet.</p>}
            </div>
        </div>
    );
};
